// Simulador do Algoritmo de Tomasulo - após tentar corrigir warnings
...
enum InstructionType { ADD, SUB, MUL, DIV, LOAD, STORE, INVALID };
...
private:
    ...
    size_t nextInstructionIndex = 0;        // Índice da próxima instrução a ser emitida.
    size_t robHead = 0, robTail = 0;        // Ponteiros para a cabeça e cauda do ROB (fila circular).
    size_t robEntriesAvailable;             // Número de entradas livres no ROB.

    // --- Parâmetros de Configuração ---
    const int ADD_LATENCY, MUL_LATENCY, DIV_LATENCY, LOAD_LATENCY, STORE_LATENCY;
    const size_t ROB_SIZE;
    ...
    struct ExecutingInstruction {
        size_t rsIndex; string rsType; int remainingCycles; size_t instructionIndex;
    };
    ...
    bool issueInstruction() {
        ...
        if (nextInstructionIndex >= instructions.size() || robEntriesAvailable == 0) {
            return false;
        }
        ...
        size_t currentRobIdx = robTail;
        ReorderBufferEntry &robEntry = rob[currentRobIdx];
        robEntry.busy = true;
        robEntry.instructionIndex = static_cast<int>(nextInstructionIndex);
        ...
        robEntry.value = 0; robEntry.address = 0; robEntry.valueReady = false; // Inicializa campos.
        ...
        if (rsInfo.second == "ADD") rs = &addRS[static_cast<size_t>(rsInfo.first)];
        else if (rsInfo.second == "MUL") rs = &mulRS[static_cast<size_t>(rsInfo.first)];
        else if (rsInfo.second == "LOAD") rs = &loadRS[static_cast<size_t>(rsInfo.first)];
        else if (rsInfo.second == "STORE") rs = &storeRS[static_cast<size_t>(rsInfo.first)];
        // Não deve ser nullptr se rsInfo.first != -1.

        rs->busy = true;
        rs->op = originalInst.type;
        rs->instructionIndex = static_cast<int>(nextInstructionIndex);
        ...
        // Operando 1 (Vj/Qj):
        if (originalInst.type == LOAD) { // Para LOAD, src1 é o offset, armazenado em 'A'.
            rs->A = atoi(originalInst.src1.c_str());
            rs->Vj = 0; rs->Qj = ""; // Vj/Qj não são usados para um operando registrador em LOAD.
        } else { // Para ADD, SUB, MUL, DIV (src1 é registrador) e STORE (src1 é registrador do dado).
            if (!originalInst.src1.empty()) { // Verifica se src1 existe (pode não existir para algumas arquiteturas/instruções)
                if (regStatus[originalInst.src1].busy) { // Se o valor de src1 está pendente.
                    ...
                    if (rob[static_cast<size_t>(producingRobIdx)].busy && rob[static_cast<size_t>(producingRobIdx)].state == ROB_WRITERESULT && rob[static_cast<size_t>(producingRobIdx)].valueReady) {
                        rs->Vj = rob[static_cast<size_t>(producingRobIdx)].value; // Pega valor direto do ROB.
                        rs->Qj = "";
                    } else {
                        rs->Qj = to_string(producingRobIdx); // Armazena a tag do ROB que fornecerá o valor.
                    }
                } else { // Valor de src1 está disponível no banco de registradores.
                    rs->Vj = registers[originalInst.src1];
                    rs->Qj = "";
                }
            } else { rs->Vj = 0; rs->Qj = "";} // Sem src1
        }
        ...
        if (originalInst.type == ADD || originalInst.type == SUB || originalInst.type == MUL || originalInst.type == DIV ||
            originalInst.type == LOAD || originalInst.type == STORE) { // Instruções que podem ter src2.
            if (!originalInst.src2.empty()) { // Verifica se src2 (registrador) existe.
                if (regStatus[originalInst.src2].busy) {
                    int producingRobIdx = regStatus[originalInst.src2].robIndex;
                    if (rob[static_cast<size_t>(producingRobIdx)].busy && rob[static_cast<size_t>(producingRobIdx)].state == ROB_WRITERESULT && rob[static_cast<size_t>(producingRobIdx)].valueReady) {
                        rs->Vk = rob[static_cast<size_t>(producingRobIdx)].value;
                        rs->Qk = "";
                    } else {
            ...
            } else { // Sem src2 explícito (ex: L.D F1, 100() implicaria base 0 ou erro de formato).
                rs->Vk = 0; // Se src2 vazio, assume-se valor 0 para base (ou deveria ser erro de formato).
                rs->Qk = "";
            }
        } else {rs->Vk = 0; rs->Qk = "";} // Instruções sem src2.
        ...
        if (originalInst.type == STORE) {
            ...
            if (rs->Qj.empty()) { // Se Vj (dado do store) está pronto.
        ...
        if (originalInst.type != STORE) {
            ...
            regStatus[originalInst.dest].robIndex = static_cast<int>(currentRobIdx);
        }
    ...
    void startExecution() {
        ...
        for (int type_idx = 0; type_idx < 4; ++type_idx) { // Itera sobre os tipos de RS
            for (size_t i = 0; i < rs_sizes[type_idx]; ++i) { // Itera sobre cada RS daquele tipo
                ...
                if (currentRS.busy && currentRS.Qj.empty() && currentRS.Qk.empty()) {
                    ...
                    for (const auto& execInst : executingInstructions) {
                        if (execInst.rsType == rs_types[type_idx] && execInst.rsIndex == i) {
                            alreadyExecuting = true; break;
                        }
                    }

                    if (!alreadyExecuting) { // Se não está executando, pode iniciar.
                        size_t robIdxForInst = static_cast<size_t>(stoi(currentRS.destRobIndex)); // Pega o índice do ROB associado.
                        ...
                        exec.instructionIndex = static_cast<size_t>(currentRS.instructionIndex);
                        
                        // Define a latência correta.
                        if (rs_types[type_idx] == "MUL") { // MUL e DIV usam a mesma RS.
    ...
    void advanceExecution() {
        for (size_t i = 0; i < executingInstructions.size(); /* não incrementa aqui ao apagar */) {
            ...
            if (executingInstructions[i].remainingCycles <= 0) { // Se a execução terminou.
                ...
                executingInstructions.erase(executingInstructions.begin() + static_cast<vector<ExecutingInstruction>::difference_type>(i));
            } else {
    ...
    void processWriteBack() {
        if (completedForCDB.empty()) return; // Nada a fazer se a fila do CDB está vazia.

        size_t originalInstIndex = completedForCDB.front(); // Pega a próxima instrução da fila.
        completedForCDB.pop();
        ...
        bool found_rs = false;
        if (!found_rs) for (size_t i = 0; i < addRS.size(); ++i) if (addRS[i].instructionIndex == static_cast<int>(originalInstIndex) && addRS[i].busy) { rs = &addRS[i]; robIndexStr = rs->destRobIndex; found_rs = true; break;}
        if (!found_rs) for (size_t i = 0; i < mulRS.size(); ++i) if (mulRS[i].instructionIndex == static_cast<int>(originalInstIndex) && mulRS[i].busy) { rs = &mulRS[i]; robIndexStr = rs->destRobIndex; found_rs = true; break;}
        if (!found_rs) for (size_t i = 0; i < loadRS.size(); ++i) if (loadRS[i].instructionIndex == static_cast<int>(originalInstIndex) && loadRS[i].busy) { rs = &loadRS[i]; robIndexStr = rs->destRobIndex; found_rs = true; break;}
        if (!found_rs) for (size_t i = 0; i < storeRS.size(); ++i) if (storeRS[i].instructionIndex == static_cast<int>(originalInstIndex) && storeRS[i].busy) { rs = &storeRS[i]; robIndexStr = rs->destRobIndex; found_rs = true; break;}
        ...
        size_t producingRobIdx = static_cast<size_t>(stoi(robIndexStr)); // Converte a string do índice do ROB para inteiro.
        ...
        switch (inst.type) {
            ...
            case DIV: 
                ...
                else { cerr << "Erro: Divisao por zero na instrucao " << originalInstIndex << "!" << endl; resultData = 0; }
                break;
            case LOAD:
                effectiveAddr = rs->A + rs->Vk; // A é offset, Vk é valor do registrador base.
                if (effectiveAddr >= 0 && effectiveAddr < 1024) resultData = memory[effectiveAddr];
                else { cerr << "Erro: Endereco de LOAD invalido (" << effectiveAddr << ") para inst " << originalInstIndex << endl; resultData = 0; } // Tratar erro de acesso.
            ...
            case STORE:
                ...
                break;
            case INVALID:
                cerr << "Erro: Tipo de instrucao invalido encontrado durante WriteBack!" << endl;
                resultData = 0;
                break;
        }

        // Atualiza a entrada do ROB com o resultado/valor e estado.
        if (rob[producingRobIdx].busy) { // Verifica se a entrada do ROB ainda é relevante.
    ...
    void updateDependentRS(size_t producingRobIdx, int resultValue) {
        ...
        for (int type_idx = 0; type_idx < 4; ++type_idx) { // Itera sobre os tipos de RS.
            for (size_t i = 0; i < rs_sizes[type_idx]; ++i) { // Itera sobre cada RS.
                ReservationStation &currentRS = rs_arrays[type_idx][i];
                if (currentRS.busy) { // Se a RS está ocupada.
                    ...
                    if (currentRS.Qj == producingTag) {
                        ...
                        if (currentRS.op == STORE) {
                            size_t storeOwnRobIdx = static_cast<size_t>(stoi(currentRS.destRobIndex)); // Índice do ROB do próprio STORE.
                            if (rob[storeOwnRobIdx].busy) { // Verifica se a entrada do ROB ainda é relevante.
    ...
    void commitInstruction() {
        ...
        if (headEntry.state == ROB_WRITERESULT) {
            ...
            Instruction &originalInst = instructions[static_cast<size_t>(headEntry.instructionIndex)];
            ...
            if (headEntry.type != STORE) { // Para ADD, SUB, MUL, DIV, LOAD
                ...
                if (regStatus[headEntry.destinationRegister].busy &&
                    regStatus[headEntry.destinationRegister].robIndex == static_cast<int>(robHead)) {
...
public:
    ...
    TomasuloSimulator(int addRSCount = 3, int mulRSCount = 2, int loadRSCount = 3, int storeRSCount = 3, int rob_s = 16) :
        ADD_LATENCY(2), MUL_LATENCY(10), DIV_LATENCY(40), LOAD_LATENCY(2), STORE_LATENCY(2), // Inicializa consts
        rob(static_cast<size_t>(rob_s)), ROB_SIZE(static_cast<size_t>(rob_s)) // Inicializa o ROB com o tamanho passado ou padrão.
    {
        ...
        robHead = 0; robTail = 0; robEntriesAvailable = ROB_SIZE;
        ...
        cycle = 0; nextInstructionIndex = 0;
    }
    ...
    bool loadInstructions(const string &filename) {
        ...
        if (!file.is_open()) {
            cerr << "Erro ao abrir arquivo: " << filename << endl; return false;
        }
        string line;
        while (getline(file, line)) {
            if (line.empty() || line[0] == '#') continue; // Ignora vazias e comentários.
            istringstream iss(line); 
            string op, p1, p2, p3; // Tokens temporários para parsing.
            iss >> op >> p1; // Lê operação e primeiro operando/destino.
            if (!p1.empty() && p1.back() == ',') p1.pop_back(); // Remove vírgula.
            
            Instruction inst; // Cria instrução a ser preenchida.
            // Parseia baseado no tipo de operação.
            if (op == "ADD" || op == "SUB" || op == "MUL" || op == "DIV") {
                if (op == "ADD") inst.type = ADD; else if (op == "SUB") inst.type = SUB;
                else if (op == "MUL") inst.type = MUL; else if (op == "DIV") inst.type = DIV;
                iss >> p2 >> p3; if (!p2.empty() && p2.back() == ',') p2.pop_back();
                inst.dest = p1; inst.src1 = p2; inst.src2 = p3;
            } else if (op == "L.D" || op == "LOAD") {
                inst.type = LOAD; inst.dest = p1; // p1 é registrador destino.
                ...
                } else { cerr << "Formato L.D invalido: " << p2 << " na linha: "<< line << endl; continue; }
            } else if (op == "S.D" || op == "STORE") {
                inst.type = STORE; inst.src1 = p1; // p1 é registrador do dado.
                iss >> p2; // p2 é "offset(base)".
                size_t openParen = p2.find('('), closeParen = p2.find(')');
                if (openParen != string::npos && closeParen != string::npos && closeParen > openParen +1) {
                    inst.dest = p2.substr(0, openParen); // inst.dest no STORE é usado para o offset.
                    inst.src2 = p2.substr(openParen + 1, closeParen - openParen - 1); // registrador base.
                } else { cerr << "Formato S.D invalido: " << p2 << " na linha: " << line << endl; continue; }
            } else {
                cerr << "Instrucao nao reconhecida: " << op << " na linha: " << line << endl; continue;
            }
    ...
    bool isSimulationComplete() const {
        ...
        if (nextInstructionIndex < instructions.size()) return false;
        ...
        if (!instructions.empty()) { // Só verifica commitCycle se há instruções.
            for(const auto& inst : instructions) {
                if(inst.commitCycle == -1) return false; // Se alguma não foi cometida.
            }
        }
    ...
    void printRegisters() const {
        ...
        for (const auto& pairReg : registers) { // Usando range-based for para clareza.
    ...
    void printStatus() {
        ...
        for (size_t i = 0; i < instructions.size(); ++i) {
            const Instruction &inst = instructions[i]; string instStr;
            switch (inst.type) {
        ...
        auto printRSGroup = [&](const string& name, const vector<ReservationStation>& rsGroup) {
            ...
            for (size_t i = 0; i < rsGroup.size(); ++i) {
                const auto& rs = rsGroup[i]; string opStr;
                if(rs.busy) switch(rs.op){ case ADD: opStr="ADD"; break; case SUB: opStr="SUB"; break; case MUL: opStr="MUL"; break; case DIV: opStr="DIV"; break; case LOAD: opStr="LOAD"; break; case STORE: opStr="STORE"; break; case INVALID: default: opStr="???"; }
                printf("| %-1zu | %-4s | %-5s | %-5s | %-5s | %-7s | %-7s | %-10s | %-3s | %-7s |\n", i, (rs.busy ? "Sim" : "Nao"), opStr.c_str(),
        ...
        for (size_t i = 0; i < ROB_SIZE; ++i) {
            ...
            if (entry.busy && entry.type == STORE && !entry.valueReady && entry.state == ROB_WRITERESULT) value_s = "(pend)"; // Dado do store pendente mas endereço pronto

            printf("| %-4zu | %-4s | %-7s | %-5s | %-11s | %-7s | %-6s | %-5s | %-7s |\n", i, (entry.busy ? "Sim" : "Nao"),
...
int main() {
    ...
    if (!simulator.loadInstructions(filename)) { // Carrega as instruções.
        cerr << "Falha ao carregar instrucoes. Finalizando." << endl; return 1;
    }
    ...
    while (!simulator.isSimulationComplete()) {
        ...
        // Limpa completamente o buffer de entrada
