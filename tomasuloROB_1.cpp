...
private:
    ...
    struct ExecutingInstruction {
        int rsIndex; string rsType; int remainingCycles; int instructionIndex;
    };
    ...
    bool issueInstruction() {
        ...
        if (originalInst.type == ADD || originalInst.type == SUB || originalInst.type == MUL || originalInst.type == DIV ||
            ...
            } else { // Sem src2 explícito (ex: L.D F1, 100() implicaria base 0 ou erro).
...
public:
    ...
    {
        addRS.resize(addRSCount); mulRS.resize(mulRSCount); loadRS.resize(loadRSCount); storeRS.resize(storeRSCount);
